...
public class WebSocketRTCClient implements AppRTCClient,
  ...
  public WebSocketRTCClient(SignalingEvents events, LooperExecutor executor, io.socket.client.Socket socket) {
    this.events = events;
  ...
  @Override
  public void connectToRoomBySocket(RoomConnectionParameters connectionParameters, String params) {
    this.connectionParameters = connectionParameters;
    this.respParams = params;

    executor.execute(new Runnable() {
      @Override
      public void run() {
        connectToRoomInternalBySocket();
      }
    });
  ...
  private void connectToRoomInternal() {
    ...
    Log.d(TAG, "Connect to room go: " + connectionUrl);
    roomState = ConnectionState.NEW;
  ...
  // Connects to room by socketio - function runs on a local looper thread.
  private void connectToRoomInternalBySocket() {
    //String connectionUrl = getConnectionUrl(connectionParameters);
    Log.d(TAG, "Connect to room go: " + this.connectionParameters.roomId);
    ...
    new RoomParametersFetcher("", null, callbacks).roomResponseParse(this.respParams);
  }
  ...
  public void sendOfferSdp(final SessionDescription sdp) {
    executor.execute(new Runnable() {
      ...
      public void run() {
        ...
        sendPostMessageBySocket(MessageType.MESSAGE, messageUrl, json.toString());
        if (connectionParameters.loopback) {
  ...
  // Send SDP or ICE candidate to a room server.
  private void sendPostMessageBySocket(
          final MessageType messageType, final String url, final String message) {
    String logInfo = url;
    if (message != null) {
      logInfo += ". Message: " + message;
    ...
    Log.d(TAG, "C->GAE: " + logInfo);
    mSocket.emit("sendInitMsg", message);
